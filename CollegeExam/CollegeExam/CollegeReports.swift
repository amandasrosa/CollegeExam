...
class CollegeReports {
    ...
    */
    func instructorsByClasses() -> [String] {
        report.append(Util.pad("Instructor", 18) + " | " + Util.pad("Class Id",10) + " | " + Util.pad("Course", 40) + " | Average Grade\n")
        for c in college.getClasses() {
            if getAverageGradeOfClasse(c) != 0 {
                ...
                report.append(Util.pad(instructor, 18) + " | " + Util.pad(classId, 10) + " | " + Util.pad(course, 40) + " | \(average) \n")
            }
        }
        return report
    }
    ...
    */
    func studentsByCountry() -> [String] {
        report.append(Util.pad("Country", 15) + "| Number of Students\n")
        ...
        for c in countries {
            ...
            report.append(Util.pad(String(describing: c), 15) + "| \(count)\n")
        }
        return report
    }
    ...
    */
    func coursesByWorstAverage() -> [String] {
        report.append(Util.pad("Course", 40) + "| Grade\n")
        ...
        report.append(Util.pad(elements[0].0.getName(), 40) + "| \(elements[0].1)\n")
        report.append(Util.pad(elements[1].0.getName(), 40) + "| \(elements[1].1)\n")
        report.append(Util.pad(elements[2].0.getName(), 40) + "| \(elements[2].1)\n")
        return report
    }
    ...
    */
    func classesByInscructorsPerWeek() -> [String]  {
        report.append(Util.pad("Instructor", 18) + " | " + Util.pad("Weekday",12) + " | Number of classes\n")
        for i in college.getEmployees() {
            for e in schedulesPerInstructor(i) {
                report.append(Util.pad(i.getName(), 18) + " | " + Util.pad(e.0, 12) + " |  \(e.1)\n")
            }
        }
        return report
    }
    ...
    */
    func studentsByBestAverage() -> [String] {
        report.append(Util.pad("Program", 20) + "| " + Util.pad("Student", 20) + "| Best Average Grade\n")
        for p in college.getPrograms() {
            var elements = getAverageGradeOfEachSudentInProgram(p).sorted(by: {$0.1 > $1.1})
            report.append(Util.pad(p.getName(), 20) + "| " + Util.pad(elements[0].0.getName(), 20) + "| \(elements[0].1)\n")
        }
        return report
    }
